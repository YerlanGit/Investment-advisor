...
`s.by_asset[].asset_delta_pct` (с применённым выпуклым ограничителем ±35%),
**никогда** не пересчитывай вручную как β × shock. Если β AVGO = 2.11 и шок
Market = −20%, не пиши «AVGO упадёт на 42%» — пиши число из движка
(после кэпа ≈ −31–32%) с тегом [Quant Engine] и пометь «после выпуклого
ограничителя».

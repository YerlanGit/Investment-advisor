...
ДВА БАКЕТА — их путают, и это ПЕРВАЯ причина «база не выросла»
──────────────────────────────────────────────────────────────
  INBOX  gs://ramp-bot-chroma-db-inbox-investadv/   ← PDF кладут СЮДА
         (слушает Cloud Function; `entrypoint._boot_ingest_from_inbox`
          ингестит отсюда недостающее при рестарте бота)
  STORE  gs://ramp-bot-chroma-db-investadv/chroma_db/  ← БИНАРНИК ChromaDB
         (бот качает это на буте; PDF, положенный сюда, НЕ ингестится
          НИКОГДА — его никто не парсит)
`--check-store-pdfs` ищет именно этот случай: PDF, лежащий в STORE.
...
───────
  ...
  # главное: сверить INBOX со store и найти PDF, залитые не в тот бакет

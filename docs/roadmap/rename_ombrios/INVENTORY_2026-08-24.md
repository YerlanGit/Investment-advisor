...
`finance/sec_edgar.py` представляется SEC адресом `@project.com`. SEC
требует рабочий контакт: при массовых запросах с нерабочего адреса доступ
блокируют. Бренд в строке обновлён, адрес — нет: подставлять настоящий
почтовый ящик за владельца нельзя. Одна строка, когда будет адрес.

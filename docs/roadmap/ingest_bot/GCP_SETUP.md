...
Подтверждено сменой состояния на живом задании 26.08:

| Поле `status` | `lastAttemptTime` | Что это |
|---|---|---|
| `code: -1` | отсутствует | попыток не было — штатное состояние нового задания |
| `{}` (пусто) | есть, свежий | **попытка прошла УСПЕШНО** |
| `code: <N>` | есть | попытка провалилась, код — причина |

🔴 **Соответствие кодов между слоями** (замерено на тех же логах):
отказ приложения **`401`** виден в логах планировщика как
**`ERROR UNAUTHENTICATED`**. То есть `UNAUTHENTICATED` у Cloud Scheduler НЕ
означает «не пустили в сервис» — запрос дошёл, и его отверг общий секрет.
«Не пустили» — это `403` в логах сервиса. Спутать их значит чинить OIDC там,
где разошёлся токен.

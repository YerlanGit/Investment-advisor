...
| **Зависимости** | Фаза 4 |
| **Оценка** | 2.5 дня |
| **Флаг отката** | `MANUAL_PORTFOLIO_ENABLED=off` — пункт меню не показывается, хендлеры отказывают |

> ## ✅ ЗАКРЫТА 2026-08-06 (`AUDIT §−72`)
>
> Артефакты: `tg_bot.ManualPortfolio` · `manual_portfolio_enabled()` ·
> `kb_manual_confirm` · `msg_manual_input` · `cb_manual_action` ·
> `finance.manual_portfolio.build_confirmation` ·
> `db_tokenomics.{save,get,delete}_manual_draft` · `tg_bot.user_slot`.
> Тесты: `tests/test_manual_fsm_flow.py`, `tests/test_manual_portfolio_identity.py`.
>
> **Что в этом плане оказалось неверным — четыре места.**
>
> 1. **§2.1 недооценил F-4.** Ветку `manual` действительно нужно поставить до
>    проверки vault — но этого МАЛО. Ниже по `cb_confirm` есть вторая развилка
>    (`if source == "freedom": … else:`), и её `else` подставляет ДЕМО-ключи.
>    Один только фикс §2.1 привёл бы к платному демо-отчёту вместо ручного.
>    Третье место — `price_source="demo" if source == "demo" else "freedom"`:
>    там ручной портфель молча поехал бы на брокерском фиде (I-12).
> 2. **§7 требует `test_manual_button_hidden_when_flag_off` — «хендлеры не
>    регистрируются».** Так делать нельзя: сообщение с кнопкой живёт в чате
>    вечно, и после выключения флага старая кнопка стала бы обходным путём.
>    Хендлеры регистрируются всегда и ОТКАЗЫВАЮТ по флагу — гард в двух местах
>    (клавиатура и обработчик), тест проверяет оба.
> 3. **§7 требует показать «покрытие из C-8» на экране подтверждения.** Этого
>    сделать нельзя по построению: C-8 считает ПОСЛЕДСТВИЯ (доля стоимости, не
>    покрытая ценами) и живёт в конвейере, после загрузки цен. На экране —
>    pre-flight Фазы 04, и он молчит, когда кэшу нечего сказать: до Фазы 09 кэш
>    ручного провайдера пуст ВСЕГДА, и строка «истории нет по 12 бумагам» была
>    бы ложью о бумагах вместо правды о холодном кэше.
> 4. **Гейт §8 «время отчёта на 25 позициях» не измерен и измерен быть не
>    может.** Живой ручной отчёт обязан ходить в Stooq (I-12), а провайдер
>    появится в Фазе 09 — мерить пока нечего. Замер на Tradernet-фикстуре дал
>    бы число ни о чём. Измерена собственная задержка ручного шага (разбор +
>    доли + pre-flight) — весь вклад этой фазы во время отчёта.
>
> **Что пришлось починить в Фазе 04 до начала работы** (`§−72`): прокси
> подменял бумагу в портфеле · маржинальный заём отвергался · разряды в строке
> кэша ломали `CASH:KZT 500 000`.
...
Счётчики из плана (885 + 22) относились к базовой линии, снятой до Фаз 3–4;
фактическая линия на день сдачи — 1300, стало 1359.

- [x] `PYTHONPATH=src python -m pytest tests/ -q` → **1359 passed, 1 xfailed**
      (зеркало деплой-образа: 1343 passed, 16 skipped, 1 xfailed)
- [x] E2E: ручной ввод → подтверждение → **явный отказ без списания токена**.
      Отчёт не строится и не может: `provider_for_source("manual")` бросает
      `ProviderUnavailable` до Фазы 09 (I-12). Совпадение набора секций с
      `freedom` (I-1) проверяется Фазой 06
- [x] `MANUAL_PORTFOLIO_ENABLED=off` → бот ведёт себя как сегодня (I-9):
      кнопки нет И обработчик отказывает (устаревшая кнопка в старом сообщении)
- [x] Слот освобождается во всех ветках ручного флоу
      (`test_slot_released_in_every_branch` — семь веток, проверка ЗАХВАТОМ)
- [x] `git diff --stat` по восьми модулям I-2 = 0
- [ ] ⛔ Время отчёта на 25 позициях — **НЕ измерено, и это не забыто**:
      измерять нечего до Фазы 09 (см. врезку в шапке). Измерена задержка
      ручного шага: `test_review_of_25_positions_is_not_a_bottleneck`
...
| **Гейт «не хуже `freedom` +20%» может не пройти из-за параллелизма Stooq** | Замечено при сверке 2026-07-27: у Tradernet `max_workers=6`, у Stooq стартовое значение **2** (S-6 — у Stooq нет опубликованных лимитов, бан IP отключит сервис целиком). На ~20 тикерах это даёт +5–8 с к загрузке — на 2–4-минутном отчёте это единицы процентов, гейт проходится с запасом. Но при ретраях и джиттере запас съедается, поэтому **замер обязателен именно на Stooq**, а не на Tradernet-фикстуре. Если гейт не проходит: сначала поднимать параллелизм по наблюдениям (осторожно, S-6), и только потом обсуждать смягчение порога — бан IP дороже 20% времени |

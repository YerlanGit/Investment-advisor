...
default `RampBot`) → `premium_payload.meta.botUsername`.
...
3. Загрузка портфеля (быстрая часть): демо-ключи или расшифрованные ключи Freedom
   (`_get_keys_sync`; `MasterKeyRotatedError` → чистый ре-онбординг без списания).

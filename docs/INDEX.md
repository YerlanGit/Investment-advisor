...
**Глубокий (2 токена)**. Цена токена (2026-07-17): **1 токен = 2 500 ₸**
(пакет 10 = 25 000 ₸; источник правды — `tg_bot.TOKEN_PRICE_KZT`, экономика —
`ECONOMICS.md`).
...
| `ingest_bot.py` | **Второй Telegram-бот — только загрузка данных** (IB-3…IB-7): приём дневного среза Stooq и файла истории, `AdminOnlyMiddleware` (fail-CLOSED, в отличие от главного бота), диагностика `/status` `/missing` `/universe` `/check` `/prune`, плановые напоминания. Отчётов не строит, токеномику не трогает. → `docs/roadmap/ingest_bot/` |
| `ingest_access.py` | Политика доступа бота-загрузчика. Отдельно от `ingest_bot`, потому что тот тянет aiogram — а fail-closed обязан проверяться и там, где aiogram нет. |
| `ingest_entrypoint.py` | Точка входа ВТОРОГО сервиса (бот данных): health-проба + маршрут плановой проверки `POST /tasks/check` (закрыт секретом, без него ОТКЛЮЧЁН) + бот. `CMD` образа — `entrypoint.py` (бот отчётов); загрузчик — отдельный Cloud Run сервис того же образа с `--command` из `cloudbuild.yaml`. Граница сервисов — манифест `tests/test_service_boundaries.py`, план физического разделения — `ARCHITECTURE_FOR_AGENTS.md §8`. |
...
| `period_returns.py` | Мульти-период (1м/3м/6м/12м/YTD), TE/IR (лог-пространство, pairwise), sparse-robust портфельная серия — маскированная композитная с по-дневной ренормализацией весов, `MIN_DAILY_COVERAGE=0.5` (F-15). **SSOT свёртки книги** `aggregate_log_returns` = ln(1 + Σw·R) — лог СУММЫ простых доходностей, не сумма логов (`§−121`), и `apply_margin_financing` (заём — по rf). |
...
| `quote_publisher.py` | Публикация базы котировок: скачать → применить → загрузить с `if_generation_match` (compare-and-swap). Два бэкенда — локальный каталог (**дефолт, офлайн**) и GCS. Курсор вне базы — единственный детектор отката. |
| `quote_ingest.py` | Цикл применения дневной дельты: четыре гарда целостности, `apply_daily`/`apply_history`, `status`, сводка для оператора. Чистый stdlib — работает без pandas. |
...
| `test_phase60_math_audit.py` | **`§−121` математика против ИСТИНЫ**: свёртка книги против капитала, посчитанного деньгами (мутация 5/5), маржа по rf, стоп Action Plan ниже входа (3000 геометрий, монотонность по мандату), BL-μ — избыточная доходность. |
| `test_phase61_report_layer_audit.py` | **`§−121` слой отчёта**: правило §−13 в прозе (непослушная модель), VaR 95% в DOM обоих рендеров, зона входа Buy в Premium, снятый `ai_action_impact`. |
| `test_phase62_mobile_real_payloads.py` | **`§−121` мобильный замер на РЕАЛЬНЫХ payload'ах**: Premium, Jinja-фолбэк и сценарный тир на 320–414; контрольный опыт на старой поломке. |
| `test_phase63_multiuser_readiness.py` | **`§−122` многопользовательская готовность**: настоящий `_run_analysis_background` со стабами на границах — источник цен в каждой стадии, LLM/RAG/рендер/загрузка вне потока loop'а, общий гейт с очередью, снятие аренд владельца, таймаут Anthropic. |
| `test_service_boundaries.py` | **Манифест сервисов** (ядро · бот отчётов · бот данных · инструменты): ни одной стрелки между сервисами, включая ленивые импорты. |
...
| Файл | Статус | Назначение |
|---|---|---|
| [`audit/AUDIT.md`](audit/AUDIT.md) | 🟢 | **НАВИГАТОР аудита, а не журнал** (расслоён в `§−91`): доска задач, строгая оценка, правила из прошлых ошибок, протокол обновления. Полные записи раундов — в `audit/rounds/`, индекс раундов — в `audit/rounds/INDEX.md` (вынесен в `§−108`). Читается ЦЕЛИКОМ и в каждой сессии, поэтому держится маленьким намеренно. |
| [`ARCHITECTURE_FOR_AGENTS.md`](ARCHITECTURE_FOR_AGENTS.md) | 🟢 | **Читаемость проекта для Claude Code + исполнительный план Арх-0…Арх-7.** Замеры 2026-08-04 (динамика долга 08-01→08-04, `analyze_all` 1 240 строк, `results{}` 41 ключ / 17 потребителей, `CLAUDE.md` ≈ 7 600 токенов/сессия, 91% тестов названы по фазам), целевая архитектура слоёв L0–L4 и детальные фазы рефакторинга с гейтами верификации (golden-фикстура, двойной прогон) — читать ПЕРЕД стартом Арх-работ. |
| [`audit/READINESS_10_USERS_2026-09-25.md`](audit/READINESS_10_USERS_2026-09-25.md) | 🟢 | **Готовность к 10+ пользователям (свежий).** Шесть скрытых дефектов, невидимых одному пользователю (LLM в event loop, второй поход за котировками, аренда после редеплоя…), модель ёмкости на 1 vCPU, оценки логика/эффективность/качество, план P0 (до запуска) / P1 / P2. Заменяет `PRODUCTION_READINESS.md` как актуальную оценку готовности. |
| [`audit/AUDIT_360_2026-09-23.md`](audit/AUDIT_360_2026-09-23.md) | 🟢 | **Сквозной аудит 23.09 (свежий) + ПЛАН ДЕЙСТВИЙ.** Четыре дефекта в ЧИСЛАХ (сумма логарифмов −3.9 пп годовой; двойной rf в BL; бесплатная маржа; стоп выше входа), правило §−13 в прозе, потерянные VaR и зона входа, фолбэк без мобильного замера, пустой гейт изоляции. §4 — план по цене ошибки с решениями, которые нужны от владельца. |
| [`audit/AUDIT_360_2026-08-17.md`](audit/AUDIT_360_2026-08-17.md) | 🗄 | **Pre-flight ручного ввода (свежий).** §1 — готовность к живому прогону и единственный незакрытый риск (глубина базы Stooq); **§2 — карта происхождения данных: что берётся у внешних источников, что вычисляется**; §3 — находки C-1…C-5, две из них в правках прошлого раунда; §4 — связность слоёв замером AST; §5 — приоритеты. |
| [`roadmap/ingest_bot/FIRST_GCS_RUN.md`](roadmap/ingest_bot/FIRST_GCS_RUN.md) | 🟢 | **Регламент ПЕРВОЙ заливки базы в GCS** (`§−103`): проверка кольца «загрузчик → отчёт», что должно быть заведено до деплоя, как читать отказы (403/404/412), на какие защиты можно положиться. Появился после того, как разбор нашёл РАЗОМКНУТОЕ кольцо в дефолтах репозитория. |
| [`roadmap/ingest_bot/GCP_SETUP.md`](roadmap/ingest_bot/GCP_SETUP.md) | 🟡 | **Ручная обвязка GCP** (`§−109`): расписание плановой проверки (IB-7) и проверка радиуса прав писателя (IB-F2). Оба — состояние ПРОЕКТА, а не образа, поэтому деплой их не заводит. Скриптом и руками; известные свойства (200 на внутренней ошибке, секрет в двух местах) названы как решения. |
| [`audit/rounds/`](audit/rounds/) | 🗄 | **Полные записи раундов аудита** (§−0.5…§−121), шесть файлов по диапазонам номеров плюс `INDEX.md` — одна строка на раунд. Имя файла = диапазон, и это проверяет гейт (`§−121`: `ROUNDS_91-76.md` держал §−76…§−120). Выделены из `AUDIT.md` в `§−91`: журнал вырос до 382 КБ, шапка до 76 КБ, одна строка до 27.6 КБ, а 14 раундов вообще не имели секции — при том, что на 10 из них ссылается боевой код. Нумерация НЕ менялась (~540 ссылок). Поиск: `grep -n "^## −45\." docs/audit/rounds/*.md`. |
| [`audit/AUDIT_360_2026-08-12.md`](audit/AUDIT_360_2026-08-12.md) | 🗄 | **Сквозной аудит по живым отчётам (снимок 12.08).** Разбор вёлся ОТ АРТЕФАКТА НАЗАД: из готовых BASE/DEEP извлечены внедрённые данные и сверены с формулами, вёрстка измерена headless-браузером. **В математике ошибок не найдено** (Base Currency, RFR, Sharpe/Sortino, VaR/CVaR, Euler, 4-Pillar, режим — всё сошлось); восемь находок A-1…A-8 — в слое показа и в промпте. §3 — мобильный замер до/после, §4 — снятые (ложные) гипотезы, §5 — план работ, §6 — строгая оценка 7.8/10. |
| [`audit/AUDIT_360_2026-07-30.md`](audit/AUDIT_360_2026-07-30.md) | 🗄 | **Сквозной аудит проекта (снимок 30.07).** Открытые дефекты A-1…A-7 (дубликат тикера роняет отчёт; NaN в количестве обнуляет метрики; I-12 fail-open), статус Manual Portfolio (2 фазы из 11), пересчёт оценок ~7.1/10, порядок работ. |
| [`audit/PRODUCTION_READINESS.md`](audit/PRODUCTION_READINESS.md) | 🗄 | Пер-подсистемные оценки готовности + GA-блокеры (снимок 07-05…07-18). Актуальная оценка — `READINESS_10_USERS_2026-09-25.md`. |
...
| [`roadmap/manual_portfolio/`](roadmap/manual_portfolio/README.md) | 🗺 | **Ручной ввод портфеля + смена источника цен (2026-07-27).** Разбор задания против кода (6 фактических ошибок, 5 внутренних противоречий, 6 находок сверх задания) + 11 фаз, по md-файлу на фазу. Начинать с `README.md`, затем `PHASE_00_CONTRACT.md` — он снимает противоречия до первой строки кода. **Юридический инвариант I-12: данные Tradernet не-клиентам запрещены → `manual` работает ТОЛЬКО на Stooq, без резерва.** С 2026-08-09 источник Stooq — **bulk-выгрузка** `stooq.com/db/` (ключ и капча не нужны), а витрина котировок из Freedom API выделена в отдельный проект → `roadmap/freedom_warehouse/`. Доставлено 6 фаз из 11. **План раунда 12.08 — [`PLAN_2026-08-12.md`](roadmap/manual_portfolio/PLAN_2026-08-12.md)** (критический путь и запреты; работы по Freedom API сюда НЕ входят). С 2026-08-10 к фазам добавлены **`PHASE_08B_PRICE_STORE.md`** (хранилище котировок: схема, ingest, мультирынок, задание по коду) и **`OPERATOR_STOOQ.md`** (регламент оператора). |
| [`roadmap/ingest_bot/`](roadmap/ingest_bot/README.md) | 🗺 | **Отдельный Telegram-бот для обновления данных (2026-08-17).** Владелец пересылает дневную выгрузку Stooq в чат → бот применяет её (`finance/stooq_ingest`, 9 правил) и публикует `prices.sqlite` в GCS. Второй Cloud Run сервис из ТОГО ЖЕ образа: логика разбора не дублируется. Ключевые решения — публикация через compare-and-swap по `generation` GCS, **бакет** (изначально планировался отдельный; владелец свёл оба бота в `ramp-bot-state` с условием IAM на префикс `stooq/` — `§−100`), fail-closed доступ и курсор вне базы как единственный детектор отката. Пользователь ОДИН, и из этого выведены четыре решения (§1.2): координация упрощается, наблюдаемость ужесточается. Фазы IB-0…IB-8, ≈6.5 дня; **IB-0…IB-8 доставлены 2026-08-20 в офлайн-режиме** (`src/ingest_bot.py`, `ingest_access.py`, `services/quote_publisher.py`, `services/quote_ingest.py`, 109 тестов, 37 мутаций; строгий разбор 20.08 нашёл 9 дефектов и 1 пустой тест — `PLAN §16`). 🟢 **Сервис РАЗВЁРНУТ и проверен живым прогоном 25–26.08** (`§−103`…`§−107`): первое онлайн-обновление базы применено, кольцо «загрузчик → отчёт» подтверждено. Открыто одно — расписание плановой проверки и подтверждение условия IAM, оба в [`GCP_SETUP.md`](roadmap/ingest_bot/GCP_SETUP.md). **План — [`PLAN_2026-08-17.md`](roadmap/ingest_bot/PLAN_2026-08-17.md)**; что ещё можно обновлять этим ботом и что нельзя — [`DATA_SURFACE.md`](roadmap/ingest_bot/DATA_SURFACE.md). |
| [`roadmap/manual_portfolio/DATA_SUFFICIENCY_STOOQ_VS_FREEDOM.md`](roadmap/manual_portfolio/DATA_SUFFICIENCY_STOOQ_VS_FREEDOM.md) | 🟢 | **Достаточность Stooq против Freedom API (2026-08-17).** Разбор ПО ВХОДАМ отчёта: что брокер даёт, а Stooq не даёт (состав книги, `Broker_Current_Price` как фолбэк, класс инструмента из поля `t`), что от источника не зависит вовсе (SEC, FRED, RAG, ИИ), где Stooq документирован ЛУЧШЕ (сплиты измерены). Вердикт: для US-книги из ликвидных бумаг — да; для KASE и AIX-нот — нет. **Паритет кода доказан тестами, паритет ДАННЫХ не измерен.** |
| [`roadmap/freedom_warehouse/`](roadmap/freedom_warehouse/README.md) | 🗺 | **Freedom API → собственная БД котировок (2026-08-09).** Отдельный проект, выделен из `manual_portfolio/`: тянет дневные котировки Tradernet в свой PostgreSQL и отдаёт их отчёту вместо похода в брокерский API. Обслуживает **только ветку `freedom`** — ручной ввод к нему не привязан и работает без него. Код сбора доставлен (`freedom-etl/`, 109+13 тестов), в `src/` не внедрён. Общая с Manual Portfolio граница — **I-14**: происхождение едет с данными через хранилище. **План раунда 12.08 — [`PLAN_2026-08-12.md`](roadmap/freedom_warehouse/PLAN_2026-08-12.md)** (FW-1…FW-4; работы по ручному вводу сюда НЕ входят). |
| [`roadmap/rename_ombrios/`](roadmap/rename_ombrios/README.md) | 🟢 | **Переименование RAMP → OMBRIOS (2026-08-24).** Карта «что трогать, что не трогать»: слово `ramp` в репозитории — ЧЕТЫРЕ несовместимых сущности (бренд · имя ресурса GCP · контракт `df.attrs['_ramp_*']` · математический хелпер `scoring._ramp`), поэтому `sed -i 's/ramp/ombri/g'` — запрещённая операция. Светофор 🟢🟡⚫🔴, полная опись с `файл:строка` ([`INVENTORY_2026-08-24.md`](roadmap/rename_ombrios/INVENTORY_2026-08-24.md)) и рунбук переезда главного бота `@KEN_investment_bot` → `@Ombri_bot` ([`MIGRATION_MAIN_BOT.md`](roadmap/rename_ombrios/MIGRATION_MAIN_BOT.md)): состояние ключуется `telegram_id` пользователя, поэтому балансы, мандат и ключи брокера переезд переживают; теряется только deep-link в УЖЕ выпущенных отчётах. Там же честный расчёт по постоянному тестовому боту (long-polling не масштабируется в ноль → второй всегда оплачиваемый инстанс; общий `_STATE_BUCKET` → второй писатель в SQLite). Оба бота переехали (`§−100`, `§−101`); имена продукта — SSOT `src/branding.py`, литерал бренда ловится тестом. Пошаговая инструкция оператора на переключение — [`OPERATOR_SWITCH.md`](roadmap/rename_ombrios/OPERATOR_SWITCH.md). **Переключение на `@Ombri_bot` — ручное и ещё не сделано:** подстановки держат прежний секрет и прежний хэндл, и переключаются ТОЛЬКО парой. |
...
| Второй бот-загрузчик, публикация `prices.sqlite` в GCS, права сервиса-загрузчика | **`roadmap/ingest_bot/PLAN_2026-08-17.md`** — границы, CAS-публикация, модель угроз, фазы IB-0…IB-8 |
| Любая правка, где встречается `RAMP`/`ramp` (бренд, токен, имя ресурса, `_ramp_*`) | **`roadmap/rename_ombrios/README.md`** — светофор «трогать / не трогать»; половина вхождений трогать нельзя |
| Обещание «отчёт как у брокера» на ручном вводе | **`roadmap/manual_portfolio/DATA_SUFFICIENCY_STOOQ_VS_FREEDOM.md`** — чем Stooq достаточен, а чем нет, и что тесты паритета НЕ доказывают |
...
| 2026-09-23 | `rounds/ROUNDS_91-76.md` разрезан: §−92…§−121 → `ROUNDS_122-92.md` | Файл держал §−76…§−120 (3 524 строки) при имени «91-76»; правило поиска «открывай файл по диапазону» врало. Теперь имя = диапазон под гейтом. |

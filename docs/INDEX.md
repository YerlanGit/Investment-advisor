...
| [`roadmap/ingest_bot/`](roadmap/ingest_bot/README.md) | 🗺 | **Отдельный Telegram-бот для обновления данных (2026-08-17).** Владелец пересылает дневную выгрузку Stooq в чат → бот применяет её (`finance/stooq_ingest`, 9 правил) и публикует `prices.sqlite` в GCS. Второй Cloud Run сервис из ТОГО ЖЕ образа: логика разбора не дублируется. Ключевые решения — публикация через compare-and-swap по `generation` GCS, **отдельный бакет** (сегодня база котировок лежит рядом с `tokenomics.db` и `users_vault.db`), fail-closed доступ. Фазы IB-0…IB-8, ≈7 дней. **План — [`PLAN_2026-08-17.md`](roadmap/ingest_bot/PLAN_2026-08-17.md)**; что ещё можно обновлять этим ботом и что нельзя — [`DATA_SURFACE.md`](roadmap/ingest_bot/DATA_SURFACE.md). Кода нет. |

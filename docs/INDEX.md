...
| Файл | Статус | Назначение |
|---|---|---|
| [`audit/AUDIT.md`](audit/AUDIT.md) | 🟢 | Живой институциональный аудит: находки, Было/Стало, дашборд (§−1…§−39). |
...
| [`audit/rounds/`](audit/rounds/) | 🗄 | **Полные записи раундов аудита** (§−0.5…§−91), пять файлов по диапазонам номеров. Выделены из `AUDIT.md` в `§−91`: журнал вырос до 382 КБ, шапка до 76 КБ, одна строка до 27.6 КБ, а 14 раундов вообще не имели секции — при том, что на 10 из них ссылается боевой код. Нумерация НЕ менялась (~540 ссылок). Поиск: `grep -n "^## −45\." docs/audit/rounds/*.md`. |
| [`audit/AUDIT_360_2026-08-12.md`](audit/AUDIT_360_2026-08-12.md) | 🟢 | **Сквозной аудит по живым отчётам (свежий).** Разбор вёлся ОТ АРТЕФАКТА НАЗАД: из готовых BASE/DEEP извлечены внедрённые данные и сверены с формулами, вёрстка измерена headless-браузером. **В математике ошибок не найдено** (Base Currency, RFR, Sharpe/Sortino, VaR/CVaR, Euler, 4-Pillar, режим — всё сошлось); восемь находок A-1…A-8 — в слое показа и в промпте. §3 — мобильный замер до/после, §4 — снятые (ложные) гипотезы, §5 — план работ, §6 — строгая оценка 7.8/10. |
| [`audit/AUDIT_360_2026-07-30.md`](audit/AUDIT_360_2026-07-30.md) | 🟢 | **Сквозной аудит проекта (свежий).** Открытые дефекты A-1…A-7 (дубликат тикера роняет отчёт; NaN в количестве обнуляет метрики; I-12 fail-open), статус Manual Portfolio (2 фазы из 11), пересчёт оценок ~7.1/10, порядок работ. |

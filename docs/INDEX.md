...
| `ingest_bot.py` | **Второй Telegram-бот — только загрузка данных** (IB-3/IB-4): приём дневного среза Stooq, `AdminOnlyMiddleware` (fail-CLOSED, в отличие от главного бота), `/status`. Отчётов не строит, токеномику не трогает. → `docs/roadmap/ingest_bot/` |
...
| `ingest_entrypoint.py` | Точка входа сервиса-загрузчика (health-сервер + бот). **Прод его НЕ запускает** — `Dockerfile` стартует `entrypoint.py`. |

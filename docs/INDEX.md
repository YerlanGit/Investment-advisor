...
| `ingest_entrypoint.py` | Точка входа сервиса-загрузчика: health-проба + маршрут плановой проверки `POST /tasks/check` (закрыт секретом, без него ОТКЛЮЧЁН) + бот. **Прод его НЕ запускает** — `Dockerfile` стартует `entrypoint.py`. |
...
| [`audit/rounds/`](audit/rounds/) | 🗄 | **Полные записи раундов аудита** (§−0.5…§−108), пять файлов по диапазонам номеров плюс `INDEX.md` — одна строка на раунд. Выделены из `AUDIT.md` в `§−91`: журнал вырос до 382 КБ, шапка до 76 КБ, одна строка до 27.6 КБ, а 14 раундов вообще не имели секции — при том, что на 10 из них ссылается боевой код. Нумерация НЕ менялась (~540 ссылок). Поиск: `grep -n "^## −45\." docs/audit/rounds/*.md`. |

...
| [`roadmap/ingest_bot/`](roadmap/ingest_bot/README.md) | 🗺 | **Отдельный Telegram-бот для обновления данных (2026-08-17).** Владелец пересылает дневную выгрузку Stooq в чат → бот применяет её (`finance/stooq_ingest`, 9 правил) и публикует `prices.sqlite` в GCS. Второй Cloud Run сервис из ТОГО ЖЕ образа: логика разбора не дублируется. Ключевые решения — публикация через compare-and-swap по `generation` GCS, **отдельный бакет** (сегодня база котировок лежит рядом с `tokenomics.db` и `users_vault.db`), fail-closed доступ и курсор вне базы как единственный детектор отката. Пользователь ОДИН, и из этого выведены четыре решения (§1.2): координация упрощается, наблюдаемость ужесточается. Фазы IB-0…IB-8, ≈6.5 дня; **IB-0…IB-8 доставлены 2026-08-20 в офлайн-режиме** (`src/ingest_bot.py`, `ingest_access.py`, `services/quote_publisher.py`, `services/quote_ingest.py`, 109 тестов, 37 мутаций; строгий разбор 20.08 нашёл 9 дефектов и 1 пустой тест — `PLAN §16`). Сервис НЕ развёрнут: шаг `deploy-ingest-bot` выключен подстановкой `_INGEST_SERVICE`, env главного бота не тронут. **План — [`PLAN_2026-08-17.md`](roadmap/ingest_bot/PLAN_2026-08-17.md)**; что ещё можно обновлять этим ботом и что нельзя — [`DATA_SURFACE.md`](roadmap/ingest_bot/DATA_SURFACE.md). Кода нет. |

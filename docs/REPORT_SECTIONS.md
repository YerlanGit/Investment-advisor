...
| **Риск-гейдж занижен** | Композит (vol/CVaR/single-name-ERC) игнорировал плечо, MaxDD и секторную концентрацию → 73%-tech книга с плечом и −43.5% просадкой = «48 · Умеренный» | `composite_risk_score` +bounded аггравторы (opt-in, keyword-only, default None → без изменений): MaxDD 20→50%=+15, сектор 50→75%=+10, плечо 1.0→1.5x=+6; **только повышают** гейдж. `analyze_all` переопределяет composite после sector/leverage; `simulate` получает те же mandate+аггравторы → verdict-гейдж == effect-before (нет рассинхрона). Концентрация — по Tech-супергруппе (`asset_taxonomy.top_sector_concentration_pct` SSOT, 73% не 59%). Живая книга 48→70 «Агрессивный». Чистая диверсифицированная без плеча — без изменений. Тесты `test_phase32_report_logic_fixes.py` |
| **Effect = мандатная ребалансировка (2026-07-18)** | Реинвест высвободившегося веса шёл в held highest-BL тех → IT-доля/риск РОСЛИ (противоречие мандату); Sharpe раздувался look-ahead'ом (0.69→1.89 при растущем риске) | `high_priority_target_weights(sector_by_ticker)`: реинвест только в диверсификаторы ВНЕ перегруженного топ-сектора, остаток → **Кэш** (снижение экспозиции) → IT/концентрация/риск ПАДАЮТ, Effect ведёт К мандату. Sharpe — FORWARD (er/vol, единая ex-ante база с картами доходности/vol), привязан к headline, дельта ±0.6 — без look-ahead. Action Plan (направление) и Effect (магнитуда+деконцентрация) дополняют друг друга |
...
| **L-13 Реинвест в непокупаемое (2026-07-19)** | Effect предлагал «Купить FFSPC6.1028.AIX +12пп» (неквотируемая структурная нота — цены только от брокера, движок её не моделирует) и «XNDU +4.1пп» (плечевой ETP с drag) — Max TRC взлетал 16.2→36.3% | `high_priority_target_weights(reinvest_blocklist=...)`: кандидаты реинвеста фильтруются от broker-priced-only активов + sparse-dropped (собирает `investment_logic`) и от плечевых ETP (`finance/leveraged.is_leveraged_etp`); если подходящих нет — весь остаток в Кэш. Тесты `ReinvestEligibilityTest` |
| **L-14 CONL — «Крипто 0%» в мандат-панели (2026-07-19)** | CONL (2× Coinbase) классифицировался «Акции США» → мандат-панель показывала Крипто 0.0% при реальной крипто-экспозиции | `gatekeeper._classify_to_asset_key`: рекурсия по underlying через `finance/leveraged.etp_info` (CONL→COIN→Крипто; `_CRYPTO_UNDERLYINGS`); мандатный гард ИИ-идей туда же. Тесты `UnderlyingClassificationTest` |
| **L-15 Пустой фильтр HOTSPOT (2026-07-19)** | Чип «HOTSPOT» в holdings показывался всегда; при 0 hotspot-позиций давал пустую таблицу «Ничего не подходит» | `deep-holdings.jsx` + `portfolio-holdings.jsx`: чип рендерится только когда `holdings.some(status==='HOTSPOT')` |
| **L-16 Шаблонность ИИ + внутренние термины (2026-07-19)** | «В худший день из 20 теряется ≈3.5%» повторялась в 3+ полях (глоссарий plain_rule + примеры в спеках полей подталкивали к копипасте); в тексте всплывали внутренние слова «счёт −5.2» (4-Pillar score) и «Ярлык "экономика растёт"» | Новое `style_rule` в ОБОИХ промптах: каждая мысль/цифра-расшифровка звучит один раз (дальше — ссылка или другой угол), бан «счёт» → «сводная оценка 4-Pillar», бан «ярлык» → «оценка фазы цикла»; глоссарий plain_rule помечен как справка, НЕ текст для вставки. Тест `AiStyleRuleTest` |

...
**Macro-overlay (gated, ON по умолч.,** выключается env `REGIME_MACRO_OVERLAY=0`): когда
включён И подан FRED-пак, GDP-рост (→ growth), безработица (→ cycle) и — с 2026-07-04 —
**инфляционные ожидания** (10Y breakeven `T10YIE`, → growth: канал политики/ставки; уровень vs
таргет 2% ⊕ темп ре-анкоринга за ~1 мес, ключ `macro_inflation_nudge`) добавляются как
**дополнительные компоненты** осей на том же масштабе, что ETF-спреды. Каждый сигнал — бленд
**уровня** и **темпа** (rate-of-change: безработица 4.1% растущая ≠ той же падающей; ожидания,
разворачивающиеся вверх, ≠ дрейфующим вниз), ограничен `±0.05` (`_MACRO_MAX_NUDGE`) — может
**наклонить**, но не **перебить** ценовой режим. С `REGIME_MACRO_OVERLAY=0` классификатор
байт-идентичен чисто ценовому.
...
макро-сигналов» + `note`. С 2026-07-05 сверка замаппена и в Премиум
(`regime.consistency` → строка «✓/⚠ …» под буллетами ИИ-сверки) — раньше была видна только в v3.
...
14. **Bank RAG (выдержки)** — ChromaDB · GS/MS/JPM PDF; cosine retrieval (semantic 0.6 ⊕
    recency 0.4). Строка-**инвентарь** (2026-07-04): показывает, сколько записок реально в базе
    и читались ли они на прогоне — `used` → «прочитано N отрывков · база: M отчётов · K чанков»;
    `no_match` → «база M/K · релевантных не найдено»; `unavailable` → «база пуста — отчёты не
    читались». Читает 3-state `rag_status` + `rag_kb_docs`/`rag_kb_chunks` из `ai_summary`
    (проброшены `tg_bot._fetch_rag_context`).
15. **ИИ-цитирование банк-аналитики** (2026-07-04, `_rag_citation_status`) — вскрывает, ССЫЛАЛСЯ
    ли нарратив на банки и КАК: проверенные `[RAG:файл]`-цитаты (`ok`) vs. банк-консенсус
    `[GS]/[Barclays]/[JPM]`, который модель по инструкции промпта выдаёт из СВОИХ знаний даже при
    пустой базе (`warn` — «из общих знаний модели, не подтверждено отчётами»). Счётчики
    `rag_file_citations`/`rag_bank_citations`/`rag_cited_banks` из `ai_narrative._count_rag_citations`.
16. **AI verdict · bullets** — Anthropic · <display-модель> (через `_model_display_name` — фикс 5.2,
    иначе сырой id «claude-sonnet-4-6»); помечен «не является ИИР».
17. **LLM-чекер: контроль галлюцинаций** + **LLM-чекер: проверка вычислений** —
...
> **Аудит:** CoVe вырос с 13 до **24 строк** (при 6 FRED-сериях): Sprint 6 добавил факторную
> независимость (κ), валютный слой, Smart-Money (gated), 2 LLM-чекера; 2026-07-04 — строку
> «ИИ-цитирование банк-аналитики» + инвентарь базы в строке Bank RAG. Подпись EWMA исправлена,
> модель — человекочитаемым именем.
>
> **Легенда/футер (2026-07-04):** панель «Контроль качества данных» больше не хардкодит зелёную
> `✓` перед каждым чипом — красит СОБСТВЕННЫЙ символ строки (`✓` sage / `✗` rust / `⚠` gold /
> `—` neutral), поэтому `✗ RAG` и `⚠ ИИ↔банк-аналитика` рендерятся честно, без двойного символа.

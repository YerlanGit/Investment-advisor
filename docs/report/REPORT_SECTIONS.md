...
| KPI-карточки CVaR / Sharpe / MaxDD | `cvar`, `cvar_ci`, `sharpe`, `sortino`, `max_drawdown`, `volatility`, `*_dollar`, `var_95_daily`, `var_dollar` | `build_payload` (KPI-блок); подпись CVaR — `premium_payload._cvar_sub` | **Композитная база (F-20…F-23)**: реализованные Sharpe/CAGR/CVaR/MaxDD считаются на full-panel masked composite (единая основа с остальными секциями); свёртка книги — ln(1+Σw·R) (`§−121`); bootstrap-CVaR Politis-Romano; χ²/Fisher-ДИ (`finance/inference.py`); флаг `var_reliability` при короткой истории. **VaR 95%** — в подписи CVaR-карточки обоих рендеров (`§−121`: считался и не показывался нигде) |
...
| Валюта сделки в карточке позиции (оба тира, −37) | `assets[].currency/fx_converted/purchase_price` → `buyPrice`/`ccy`/`fxConverted` | `build_payload._fmt_purchase_price` → `premium_payload._map_base`/`_map_deep` | `MAC3RiskEngine.fx_rate_to_base` (`MATH_ENGINE.md §1.2a`). Стоимость, веса и риск — ВСЕГДА в базовой валюте; цена покупки печатается «23.08 USD (12 000 ₸)». Чип панели целостности появляется только при непустом `results["fx_converted_rows"]` |
...
| Уровни (зоны, тейк, стоп) | `buy_zone` → Premium `actionPlan[].entry` (второй строкой под ценой) | `premium_payload` | `compute_levels`: ATR+SMA+52w-high; mandate-scale ATR-дистанций 0.75/1.0/1.25 (`MANDATE_LEVEL_SCALE`); SMA-якоря без масштаба. **`§−121`:** стоп Buy — от ВХОДА (нижней границы зоны), SMA-якоря — только ниже рынка, зона выше рынка клипается (`MATH_ENGINE.md §14a`) |
| Турновер-кап | — | — | `MAX_TRADE_BLOCK_PORTFOLIO_PCT=0.25` (+ BL `max_active_share` по мандату) |
| AI-комментарий плана | `ai_action_comment`, `action_plan_text` | прокидка из `ai_summary` | обязан вести нарушенную долю обратно в мандатный лимит (`mandate_rule`, §5). `ai_action_impact` **снят** (`§−121`): просил модель СЧИТАТЬ эффект плана, который уже считает движок (§7); ключ контракта пуст до решения владельца |
...
| Качество данных · замечания (Ф-3) | `data_quality{profile,findings[]}` → CoVe-строка | `finance/data_checks` (блоки B+C, вызов в `analyze_all`) → `data_lineage._data_quality_status` | **УСЛОВНАЯ** строка: появляется только при DEGRADE-находках (реестр сжат до 16 — всегда-включённую добавлять нельзя). BLOCK сюда не попадает по построению: он выбрасывает `DataQualityBlocked`, и отчёта нет. `AUDIT §−49` |

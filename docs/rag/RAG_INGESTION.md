...
- **Рез идёт ПО ГРАНИЦЕ ТЕКСТА** (абзац → конец предложения → перевод строки →
  слово), а не по 1200-му символу. Замер `§−97`: раньше три чанка из пяти
  открывались с середины фразы и столько же ими обрывались — эти обрывки чинил
  постфактум слой отчёта, теряя текст.
- **Секцию начинает заголовок ЛЮБОГО уровня** (`#`…`######`). Прежний `#{1,3}` не
  видел `#### …`, и подраздел вливался в соседнюю секцию с ЧУЖИМ именем — а имя
  секции печатается в подписи выдержки.
- **Порог отсеивает бессодержательное, а не короткое.** Прежние 150 символов
  выбрасывали секцию целиком: «### Rates / Duration risk is back.» не попадал в
  базу вообще, хотя у банковского отчёта короткая секция — самый резкий тезис.
  Сейчас отбрасываются голый заголовок, номер страницы, «Source: …» без текста.
- **Отрывки считает `count_snippets`** — по заголовкам собранного контекста.
  Прежняя формула считала непустые абзацы markdown и печатала «прочитано 97
  отрывков» при шести запрошенных (`§−97` E-2).

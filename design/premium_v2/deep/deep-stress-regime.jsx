      ...
      <div className="glass-strong rounded-4xl p-6 shadow-card lift h-full flex flex-col">
        ...
        <div className="space-y-1.5 flex-1">
          ...
          {r.drivers.map((d,i) => {
            ...
            return (
            <div key={i} className="grid grid-cols-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(6.5rem,1fr)_auto_auto_auto] items-center gap-x-2 gap-y-0.5 sm:gap-3 py-1.5 border-b border-ink-900/5 last:border-0">
              <span className="text-[11.5px] text-ink-700 leading-snug break-words">{d.name}</span>
              <span className={`text-[11.5px] num font-semibold whitespace-nowrap justify-self-end sm:justify-self-auto ${tt}`}>{d.val}</span>
              <span className={`text-[9.5px] font-mono whitespace-nowrap ${d.trendTone === 'flat' ? 'text-ink-400' : tt}`}>{d.trend}</span>
              <span className={`text-[8.5px] font-mono font-bold tracking-wider px-1.5 py-0.5 rounded-full whitespace-nowrap justify-self-end sm:justify-self-auto ${d.tone==='warn'?'bg-gold-400/18 text-gold-700':'bg-sage-500/12 text-sage-600'}`}>{d.state}</span>
            </div>
            );
          })}
        </div>

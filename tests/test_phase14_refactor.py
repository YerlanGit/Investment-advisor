...
class ConsolidationTest(unittest.TestCase):
    ...
    def test_classify_asset_class_canonical(self) -> None:
        ...
        cases = {
            ...
            # A-5 (2026-08-02): было «Акции KZ» — и живой отчёт печатал
            # AIX-ноту как долевую бумагу, тогда как панель мандата считала
            # её «Bonds», а риск-модель проксировала на `VWOB.US` (EM govt
            # bond, §−38).  Один инструмент — один ответ.  Структурная
            # природа не потеряна: `broker_api._classify_instrument` держит
            # для неё отдельное «Структ.нота» в колонке `Asset_Type`.
            "FFSPC6.1028.AIX": "Облигации",

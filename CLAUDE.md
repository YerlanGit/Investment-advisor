...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 1263 passed, 1 xfailed
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, в нём НЕТ каталога `design/`:
  ...
  PYTHONPATH=src python -m pytest tests/ -q        # → 1247 passed, 16 skipped, 1 xfailed
  ```
...
- Claude ПУШИТ, но НЕ МЕРЖИТ. Маршрут один: ветка `claude/*` → push → PR → merge жмёт человек.
  Запрещено: пуш в `main`, `gh pr merge` в любом виде, auto-merge, force-push в `main`.
  Конфликт с `main` Claude разрешает в СВОЕЙ ветке и пушит; merge всё равно за человеком.

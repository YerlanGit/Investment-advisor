...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 2000 passed, 2 xfailed (+7 skipped без playwright)
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, без `design/`: `cp -r src tests
  SYSTEM_PROMPT.md requirements*.txt <tmp>/ && cd <tmp>`, тот же прогон → 1879 passed,

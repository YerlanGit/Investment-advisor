...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 1936 passed, 2 xfailed (+7 skipped без playwright)
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, без `design/`: `cp -r src tests
  ...
  106 skipped, 2 xfailed. Зелёный GitHub CI НЕ означает, что деплой пройдёт: CI видит

...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 1939 passed, 2 xfailed (+7 skipped без playwright)
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, без `design/`: `cp -r src tests
  ...
  109 skipped, 2 xfailed. Зелёный GitHub CI НЕ означает, что деплой пройдёт: CI видит

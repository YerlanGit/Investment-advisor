...
- Бот-загрузчик (`ingest_bot`/`ingest_access`/`services/quote_*`) — ПРИСТРОЙКА:
  прод (`entrypoint`, `tg_bot`) его НЕ импортирует, иначе деплой главного бота
  начнёт зависеть от него (`test_phase51_ingest_bot.IsolationTest`).
  Дефолт хранилища — офлайн (`QUOTES_BACKEND=local`); дельта не заводит бумаг.

...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 1990 passed, 2 xfailed (+7 skipped без playwright)
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, без `design/`: `cp -r src tests
  SYSTEM_PROMPT.md requirements*.txt <tmp>/ && cd <tmp>`, тот же прогон → 1869 passed,
  128 skipped, 2 xfailed. Зелёный GitHub CI НЕ означает, что деплой пройдёт: CI видит

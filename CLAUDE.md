...
- **Чанк RAG режется по границе фразы**, секцию начинает заголовок ЛЮБОГО уровня, порог отсеивает бессодержательное, а не короткое (`§−97` E-4).
  Отрывки считает `rag_engine.count_snippets` (по заголовкам, не по абзацам).
- **Имя эмитента — ОДИН реестр** (`rag_engine.BANK_*`), новый банк = одна строка. Копий было четыре, пять банков стали `Unknown` на `wells_fargo_*.pdf`: `_` для `\b` СЛОВНЫЙ (`§−95`).
  Короткие формы (`MS`/`GS`) — только имя файла и тег, в прозе нельзя (`§−14` C-8).
...
- **Книга сворачивается ТОЛЬКО `period_returns.aggregate_log_returns` = ln(1 + Σw·R)**: Σw·ln(1+R) — средний рост бумаг, минус доходность диверсификации (демо −3.9 пп, `§−121`). Гейт — капитал, посчитанный деньгами.
...
Свежий сквозной аудит — `docs/audit/AUDIT_360_2026-09-23.md` (Было/Стало `§−121` +
ПЛАН с решениями владельца; сверка с ИСТИНОЙ, а не с формулой). Предыдущие —
`AUDIT_360_2026-08-17.md` (КАРТА ПРОИСХОЖДЕНИЯ ДАННЫХ) и `AUDIT_360_2026-08-12.md`
(разбор ОТ АРТЕФАКТА: живые BASE/DEEP + мобильный замер + строгая оценка).

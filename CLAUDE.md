...
```bash
PYTHONPATH=src python -m pytest tests/ -q          # → 1463 passed, 2 xfailed
```
...
- **Прогонов ДВА.** Второй — зеркало деплой-образа, в нём НЕТ каталога `design/`:
  ...
  PYTHONPATH=src python -m pytest tests/ -q        # → 1410 passed, 53 skipped, 2 xfailed
  ```
  Зелёный GitHub CI НЕ означает, что деплой пройдёт: CI видит полный чекаут,
  Cloud Build — только образ. Разница 53 теста — ровно те, что читают

...
## Round 9 (2026-06-29) — DEEP: перенос гейджа, графики KPI, фундаментал, факторы, план

Зеркалит раунд 8 для DEEP-отчёта плюс DEEP-специфика. Файлы: `deep/deep-overview.jsx`,
`deep/deep-factors.jsx`, `deep/deep-holdings.jsx`, `deep/deep-plan.jsx`, маппер `_map_deep`, движок
`tg_bot._build_kpi_sparklines` / `finance/sec_edgar.py`.

**Обзор (`deep-overview.jsx`):**
- **«Индекс риска» перенесён в hero** — компактный `HeroGaugeCard` (гейдж 150 + tier + ожид. доходность/Sharpe)
  в правую колонку под hero-статами; из сетки `GaugeCard` убран, ряд стал `Verdict (lg:7) + Mandate (lg:5)`.
- **KPI-карты получили график** (был пустой бокс «здесь должны быть графики»): `KpiCard` рисует тренд 12 мес
  через `<Sparkline points={k.pts}>` крупным блоком; фоллбэк на серверный `k.svg`, затем «нет истории».
- `HeroStat` `small`, `clamp()`-заголовок, правая колонка `w-full <lg` — как в BASE.

**Движок KPI:** `tg_bot._build_kpi_sparklines` теперь возвращает И сырые серии (`cvar_pts/sharpe_pts/mdd_pts`,
×100 для %-метрик) рядом с SVG; маппер `_kpi` берёт `kpi_sparklines.<metric>_pts` → `pts`. `html_renderer`
mock тоже отдаёт `_pts` (smoke).

**Факторы (`deep-factors.jsx`, Note 4+5):**
- **Колонка «Рынок» → «S&P 500»** + пояснение-сноска: в этой модели Market = S&P 500, поэтому эталон по
  построению имеет β_Market=1, стилевые ≈0 (их экспозиция «зашита» в рынок); полезный сигнал — колонка `Наклон Δ`
  (активный перекос). Данные НЕ менялись — они корректны под моделью; вопрос пользователя снят пояснением.
- **F·V·T·C раскрыты** → Fundamentals · Valuation · Technical · Credit + блок `PillarLegend` с реальными
  показателями каждого столпа (из `finance/scoring.py`): F = ROE/опер.маржа/рост выручки/долг/FCF; V = P/E,P/S;
  T = SMA50/200,RSI,MACD; C = Altman-Z/покрытие/долг.
- **Дубль ИИ-комментария устранён**: `scoresNote` дублировал `scoresAI` (один `ai_4pillar_comment` дважды) →
  маппер шлёт `scoresNote=""`, компонент его не рисует. Это и есть «почему повторяются комментарии».

**Холдинги (`deep-holdings.jsx`, Note 3):**
- Понятные подписи + тултипы: ROE · **Опер. маржа** · **Долг / Активы** · **Рост выручки** · **ATR · день** ·
  Altman-Z (каждая с `title`-расшифровкой).
- **Короткий вывод по фундаменталу** (`fundNote`) — правило-ориентированная фраза из метрик SEC (без LLM,
  без finance-math): «Сильные стороны: высокая рентабельность капитала (ROE 76%), …; риски: …».
- **Долг/Активы = 0% починен в движке**: `sec_edgar` не находил тег `Liabilities` у части эмитентов (ORCL) →
  Debt/Assets=0. Добавлен фоллбэк по балансовому тождеству: `Liabilities = Assets − Equity`, когда прямого тега нет.

**План (`deep-plan.jsx`, Note 6):** добавлена колонка **«Количество»** — units (`qty_delta`) + изменение веса
(`delta_w_pp`, пп), цвет по знаку. Маппер пробрасывает `qty`/`dw` из payload `action_plan`.

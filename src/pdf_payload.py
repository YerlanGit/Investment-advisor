    ...
    if statuses and all(s == "disabled" for s in statuses):
        return {
            ...
            "hint":     "Включается флагом SMART_MONEY_INSIDERS=1 + провайдер Form-4 (EDGAR). "
                        "Архитектура и модели — см. docs/math/SMART_MONEY.md.",
        }
    rows: list[dict] = []
...
def _build_integrity_checks(results: dict,
    ...
    # строка обязана быть названа — молча потерять позицию нельзя.
    _merged = results.get("merged_positions") or []
    if _merged:
        _txt = " · ".join(f"{m.get('ticker')} ({m.get('rows')} строки)" for m in _merged)
        checks.append({"status": "⚠", "label": "Склейка дубликатов",
                        "detail": (f"одна бумага пришла несколько раз: {_txt} — количество "
                                   "суммировано, цена покупки средневзвешенная")})
    _dropped = results.get("dropped_rows") or []
    if _dropped:
        _txt = " · ".join(f"{d.get('ticker')} — {d.get('reason')}" for d in _dropped)
        checks.append({"status": "⚠", "label": "Отброшенные строки",
                        "detail": f"не участвуют в расчёте: {_txt}"})

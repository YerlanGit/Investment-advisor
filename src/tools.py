...
from .finance.tool_plugins import FINTECH_PLUGINS
...
# Добавляем финтех-плагины в список инструментов
for tool_name, tool_info in FINTECH_PLUGINS.items():
    PORTED_TOOLS = PORTED_TOOLS + (PortingModule(
        name=tool_name,
        responsibility=tool_info["responsibility"],
        source_hint=tool_info["source_hint"],
        status="implemented",
    ),)
...
def execute_tool(name: str, payload: str = '') -> ToolExecution:
    # Делегируем финтех-плагинам (чистая Plugin Architecture)
    if name in FINTECH_PLUGINS:
        handler = FINTECH_PLUGINS[name]["handler"]
        return handler(name, payload)

    # Оригинальный код upstream для всех остальных инструментов

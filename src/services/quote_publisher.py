...
class GcsQuotePublisher:
    ...
    def _cache_limits(self) -> tuple[Optional[Path], int]:
        ...
        🔴 `§−118`: копий базы в пике не две, а ЧЕТЫРЕ — кэш (живёт всегда),
        рабочая копия команды, буфер выгрузки обратно в GCS и, на `/prune`,
        вторая копия от `VACUUM`. Именно этот счёт не сошёлся с прежним лимитом
        512 МиБ и терял ответы на чистке и на 02.09 (`§−116`: теперь 1 ГиБ).
        150 МБ по умолчанию отключают кэш там, где база переросла потолок и
        кэш стал опаснее своей пользы.
        """

...
def _ai_status(ai_summary: Optional[dict]) -> dict:
    ...
    try:
        from pdf_payload import model_display_name
        model_disp = model_display_name(model_id) or model_id
    except Exception:
        model_disp = model_id
    return _row(
        name   = "AI verdict · bullets",
...
def _fx_status(results: dict) -> dict:
    ...
    if not fx:
        # 🔴 Аудит 2026-08-12: ПУСТОЙ список конверсий — это ДВА РАЗНЫХ факта,
        # и раньше оба печатались как один благополучный.
        #   (а) конвертировать действительно нечего — вся книга уже в базовой
        #       валюте;
        #   (б) валюта, отличная от базовой, В КНИГЕ ЕСТЬ, но курса к ней НЕ
        #       ДАЛИ — записи о конверсии не появилось именно потому, что
        #       конверсия не состоялась.
        # Живой отчёт 12.08 был случаем (б): книга держала строку в KZT, чекер
        # C-11 отдельной строкой сообщал «Нет курса для валют: KZT», а эта
        # панель рядом уверенно заявляла «все активы в USD» и горела зелёным.
        # Отсутствие доказательства печаталось как доказательство отсутствия —
        # на балансе в ₸2.5 млн та же строка соврала бы на порядок (§−45).
        # Различаем по находке движка, а не по собственной догадке.
        _missing = [f for f in ((results.get("data_quality") or {}).get("findings") or [])
                    if isinstance(f, dict) and str(f.get("id")) == "C-11"]
        if _missing:
            _msg = str(_missing[0].get("message") or "").strip().rstrip(".")
            return _row(
                name   = "Валютный слой (конверсия + ставка)",
                source = f"Base Currency = {ccy} · {rfr_src}",
                method = (f"конверсия НЕ ВЫПОЛНЕНА — курса нет · {rfr_txt}"),
                status = "warn",
                note   = (f"{_msg}; позиции в этой валюте учтены по номиналу, "
                          f"как если бы это был {ccy}"),
            )
        return _row(
            name   = "Валютный слой (конверсия + ставка)",
            source = f"Base Currency = {ccy} · {rfr_src}",

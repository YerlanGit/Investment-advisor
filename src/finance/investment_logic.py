"""Ядро квант-движка MAC3/Barra: факторная модель, риск, сборка `results{}`.

Роль : посчитать ВСЁ, что потом печатает отчёт — беты, ковариацию, Euler-вклады,
       CVaR, форвардную доходность, скоринг, стресс, BL-таргеты — и отдать это
       одним словарём. Числа рождаются здесь и больше нигде.
Слой : **L2 (Engine)**. Смотрит вниз — в `finance/*` (L1/L0), `services/*`,
       `freedom_portfolio/*`. Наверх (`pdf_payload`, `tg_bot`) — НИКОГДА.
Док  : **`docs/math/MATH_ENGINE.md`** — SSOT всех формул, обновляется ВМЕСТЕ
       с любой правкой математики. Инварианты движка — `src/finance/CLAUDE.md`.

Что внутри (файл большой — вот его карта)
-----------------------------------------
* `MAC3RiskEngine` — рыночные данные, FX, прокси неликвидных бумаг, факторная
  регрессия Ridge, ковариация EWMA(63)⊕Ledoit-Wolf с PSD-проекцией,
  Euler-декомпозиция, bootstrap-CVaR. Главный метод — `calculate_structural_risk`.
* `UniversalPortfolioManager` — оркестратор пользовательского сценария:
  нормализация портфеля, чекеры качества, вызовы движка и всех аналитических
  слоёв, сборка результата. Главный метод — `analyze_all`.
* `MarketDataPreview` — лёгкая выжимка для превью портфеля в боте.

Выход: `analyze_all` → **`AnalyzeResults`** (35 ключей, форма описана в
`finance/contracts.py`, сверяется `tests/test_contracts_results.py`). У функции
РОВНО ОДИН контрактный `return`; ключи присутствуют всегда, отсутствие данных
выражается значением (`None` / пустой контейнер), а не отсутствием ключа.

Инварианты, которые ломаются незаметно
--------------------------------------
* **Ортогонализация (BLOCK 3.5) и её β̂ сбрасываются в начале `analyze_all`** —
  иначе стресс residualize-ится на бетах прошлого прогона (F-1).
* **Прокси (`proxy_for`) влияет ТОЛЬКО на риск.** Цена и P&L — всегда реальный
  тикер; проксированная бумага при этом ОСТАЁТСЯ в фактор-модели
  (`MATH_ENGINE.md §2.3a`).
* **Колонки в `calculate_structural_risk` дедуплицируются** (`dict.fromkeys`):
  портфель, держащий факторный ETF, иначе не строит отчёт вовсе. А в
  `valid_resolved` дубликаты, наоборот, сохраняются намеренно.
* **Разреженные бумаги исключаются из модели, но их вес разбавляет риск** —
  одна молодая бумага не должна ронять книгу целиком (F-15/F-21).
* **Окно чекеров качества обязано равняться `_last_regression_nobs`** — строгое
  пересечение колонок схлопывается одним молодым листингом (`AUDIT §−51`).
* Классификация инструмента — только через `finance/asset_taxonomy` (SSOT).

Перед правкой: прочитай `MATH_ENGINE.md` по своему разделу и `finance/CLAUDE.md`;
после правки — оба обязательных прогона тестов (см. корневой `CLAUDE.md`).
"""
import os
import hashlib
import pandas as pd

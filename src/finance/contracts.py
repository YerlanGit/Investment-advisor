...
Зачем
-----
`analyze_all` возвращает словарь из 35 ключей; его читают 8 модулей. До этого
...
* Частичные словари в моках (`src/report_mocks.py`) и тестах контрактом НЕ
  являются — они специально урезаны и под эту аннотацию не подпадают.
"""
...
class AnalyzeResults(TypedDict):
    ...
    Потребители (замер 2026-08-04, по обращениям к ключам):
    `pdf_payload` (30) · `ai_narrative` (16) · `finance/data_lineage` (15) ·

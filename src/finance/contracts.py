...
Зачем
-----
`analyze_all` возвращает словарь из 36 ключей; его читают 9 модулей. До этого
...
class AnalyzeResults(TypedDict):
    """Возврат `UniversalPortfolioManager.analyze_all` — 36 ключей.

    Потребители — **8 модулей** (замер 2026-08-04, по обращениям к ключам):
    `pdf_payload` (30) · `ai_narrative` (15) · `finance/data_lineage` (15) ·
    `finance/portfolio_series` (6) · `tg_bot` (5) · `agent/gatekeeper` (4) ·
    `finance/scenario_report` (4) · `report_charts` (3).

    `report_charts` появился в списке после Арх-5: билдеры графиков переехали
    туда из `tg_bot` и унаследовали его чтения (`tg_bot` при этом 6 → 5).

    🔴 Правило «замер без исполняемого гейта разъезжается», записанное здесь же,
    сбылось ВТОРОЙ раз: аудит 26.08 нашёл в списке `agent/advisor_bot` — шесть
    объявленных чтений при НУЛЕ импортёров во всём репозитории — модуль умер, а контракт продолжал
    числить его потребителем и завышать радиус изменения `analyze_all` с 8 до 9.
    Теперь у утверждения есть гейт: `ConsumerListCountsOnlyLIVEModulesTest`
    проверяет, что каждый названный потребитель ЖИВ. Обращения к ключам он
    НЕ пересчитывает сознательно — это тот самый лукавый счётчик, от которого
    отказалась `TEST_MAP`; цифры в скобках остаются замером на дату.
    ...
    #: Откуда взят СОСТАВ книги: "freedom" (брокер), "demo" (витрина) или
    #: "manual" (ручной ввод). Нужен CoVe: при ручном вводе количество и цена
    #: покупки не подтверждены брокером, и читатель обязан это знать —
    #: остальные строки провенанса отвечают на другой вопрос (`MP-04 ЧК-04.5`).

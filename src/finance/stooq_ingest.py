...
def _apply_partial_download_guard(batch: IngestBatch,
    ...
    if not 0 < us_rows < threshold:
        return
    # 🔴 §−105: под одним отказом жили ДВЕ РАЗНЫЕ причины, и лечение у них
    # противоположное (тот же класс, что `§−94`).
    #
    #   • НЕДОКАЧАН — скачалась ЧАСТЬ файла: строк много, но не все. Лечится
    #     повторным скачиванием ТОГО ЖЕ файла.
    #   • НЕ ТОТ ФАЙЛ — строк на порядок меньше порога: это выгрузка одной
    #     бумаги или одной площадки, а не дневной срез всего рынка. Повторное
    #     скачивание того же файла НЕ ПОМОЖЕТ НИКОГДА, и совет «скачайте
    #     заново» отправляет оператора по кругу.
    #
    # Живой повод (владелец, 25.08): файл 17 КБ ≈ 240 строк при полном срезе
    # США ≈ 850 КБ / ~12 000 строк. Назвать это «недокачанным» значит послать
    # человека качать заново то, что заново качать бессмысленно.
    if us_rows * 5 < threshold:
        batch.fatal = (
            f"принято {us_rows} US-баров при пороге {threshold} — это на "
            "порядок меньше дневного среза рынка, то есть НЕ ТОТ ФАЙЛ. "
            "Дневной срез всего рынка США весит ≈700–850 КБ и содержит "
            "~12 000 строк; выгрузка одной бумаги или одной площадки сюда не "
            "подходит. Скачивать заново тот же файл бесполезно — нужен "
            "дневной файл всего рынка со stooq.com/db/. База не тронута")
    else:
        batch.fatal = (
            f"принято всего {us_rows} US-баров при пороге {threshold} — "
            "файл выглядит недокачанным; скачайте его заново целиком. "
            "База не тронута")
...
def parse_history_file(path, *, window_days: int,
    ...
    for row in _read_rows(path):
        ...
        if bar.trade_date < cutoff:                    # правило 8
            ...
            continue
        batch.bars.append(bar)
    _apply_single_paper_guard(batch)                   # правило 10
    return batch


def _apply_single_paper_guard(batch: IngestBatch) -> None:
    """Правило 10: файл истории — это история ОДНОЙ бумаги.

    🔴 Повод — живой инцидент 31.08 (`AUDIT §−114`). Браузер сохранил повторную
    загрузку дневного среза как `20260831_d 2.txt`; строгое имя `YYYYMMDD_d.txt`
    не совпало, бот принял файл за историю, а история имеет право заводить
    бумаги — и завёл **11 281** бумагу с одним баром каждая, опубликовав базу.
    Это ровно тот отравленный состав, от которого `§−88` защитил дневную
    дельту, — но через другой вход.

    Признак однозначный и не зависит от имени: у файла истории один символ во
    всех строках, у дневного среза — тысячи. Проверка идёт по СОДЕРЖИМОМУ,
    поэтому закрывает и любое другое искажение имени, которого мы ещё не
    видели. Совет в сообщении конкретный: дата берётся из самого файла.
    """
    symbols = {bar.symbol for bar in batch.bars}
    if len(symbols) <= 1:
        return
    top_date, top_n = collections.Counter(
        bar.trade_date for bar in batch.bars).most_common(1)[0]
    batch.fatal = (
        f"в файле {len(symbols)} разных бумаг — это не файл истории, а дневной "
        f"срез с испорченным именем ({top_n} из {len(batch.bars)} баров "
        f"датированы {top_date}). Применить его как историю значит завести в "
        f"базе {len(symbols)} бумаг с одним баром каждая и схлопнуть окно "
        f"регрессии всей панели (AUDIT §−88). Переименуйте файл в "
        f"{top_date}_d.txt и пришлите заново. База не тронута")

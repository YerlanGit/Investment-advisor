...
# 🔴 Ключи обязаны СОВПАДАТЬ с `SeriesSpec.key` из `services.macro_data`
# (`MACRO_SERIES_CATALOG`). §−92: здесь стоял выдуманный `hy_oas`, тогда как
# каталог зовёт ряд `hy_credit_spread`. Драйвер кредитного спреда молча уходил
# в `flat` — то есть ровно тот индикатор, ради которого правило и писалось,
# остался нейтральным. Тест `§−90` этого не поймал, потому что сверял таблицу
# САМУ С СОБОЙ; теперь она связана с каталогом (`test_phase50…`).
MACRO_DRIVER_DIRECTION: dict[str, int] = {
    "yield_curve_10y2y":   +1,   # круче кривая → уходит инверсия → риск-он
    "hy_credit_spread":    -1,   # шире кредитный спред → риск-офф

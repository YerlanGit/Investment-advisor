...
def build_action_plan(*,
    ...
    for _, row in iter_df.iterrows():
        ...
        if _bl_contradicts:
            ...
            # 2026-08-03: было «BL расходится с сигналом (+6.2пп)».
            # «BL» — внутреннее сокращение (Black-Litterman), читателю оно
            # ничего не говорит, а строка не объясняла ГЛАВНОГО: почему в
            # колонке количества стоит «—». Правило `style_rule` запрещает
            # внутренние термины в тексте отчёта — здесь оно нарушалось.
            reason_bits.append(
                f"оптимизатор предлагает обратное ({delta_w_pp:+.1f} пп) — "
                "количество не указываем")
        # Аудит 2026-08-12: сделка МЕНЬШЕ ОДНОЙ АКЦИИ.  `qty_delta` округляется
        # к целым штукам, и на мелком Δw это даёт ровно 0 — вид печатает такую
        # строку как «—», то есть неотличимо от честного «количество не
        # определено».  В живом отчёте 12.08 GOOGL с чипом BUY получил пустое
        # количество и НИ ОДНОГО объяснения, тогда как у каждого другого «—»
        # причина была.  Дефект тот же, что чинил `R-6`, источник другой: не
        # расхождение с оптимизатором, а масштаб сделки.
        elif qty_delta == 0 and abs(delta_w_pp) > 1e-9 and price > 0:
            reason_bits.append(
                "сделка меньше одной акции по текущей цене — "
                "количество не указываем")
